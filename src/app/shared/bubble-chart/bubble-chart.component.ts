...
export class BubbleChartComponent implements OnChanges, OnInit {
  ...
  ngOnChanges() {
    if (this.chartData.length > 0) {
      ...
      for(let i=1; i<=this.numOfLinesX; i++) {
        ...
        if(i == this.xAxis) 
        {
          ...
          // Label for X-Axis
          this.fillText('Salary', '15px', (this.canvasWidth/2) + 50, this.widthBtwLinesX*i + 50);
        }
        else {
          this.context.strokeStyle = "#e9e9e9";
        }
        this.drawLine(0, this.widthBtwLinesX*i, this.canvasWidth, this.widthBtwLinesX*i);
      }
      ...
      for(let i=1; i<=this.numOfLinesY; i++) {
        ...
        if(i == this.yAxis) 
        {
            ...
            this.fillText('Head Count', '15px', -250, 100);
            this.context.restore();
        }
        else {
            this.context.strokeStyle = "#e9e9e9";
        }
        this.drawLine(this.widthBtwLinesY*i, 0, this.widthBtwLinesY*i, this.canvasHeight);
    }
    
    // // Translate to the new origin. Now Y-axis of the canvas is opposite to the Y-axis of the graph. So the y-coordinate of each element will be negative of the actual
    this.context.translate(this.yAxis*this.widthBtwLinesY, this.xAxis*this.widthBtwLinesX);
    ...
    for (let i=1; i<=this.maxNumOfLinesY; i++) {
      ...
      this.fillText(this.xAxisStartingPoint + ((i-1)*100), '9px', this.widthBtwLinesY*i-2, 15)
    }
    ...
    for(let i=1; i<=this.maxNumOfLinesX; i++) {
      ...
      this.drawLine(-3, -this.widthBtwLinesX*i, 3, -this.widthBtwLinesX*i)
      this.fillText(this.yAxisStartingPoint + ((i-1)*100), '9px', -20, -this.widthBtwLinesX*i+3)
    }
      ...
      this.chartData.forEach((data: any) => {
        ...
        this.fillText(data.title, '10px', salary, -headcount)
      })
    }
  }
  ...
  fillText(text: string, fontSize: any, x: any, y: any) {
    this.context.beginPath();
